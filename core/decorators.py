...
def role_required(*permissions):
    """
    Check if the user's role has all specific permissions.
    Usage: @role_required('view_student', 'view_enrollment')
    """
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            # We use .filter(codename__in=permissions).count() to check for multiple values efficiently
            if request.user.role:
                assigned_count = request.user.role.permissions.filter(
                    codename__in=permissions
                ).distinct().count()
                
                # Check if the user has ALL of the requested permissions
                if assigned_count == len(permissions):
                    return view_func(request, *args, **kwargs)
            
            # 4. Access Denied
            raise PermissionDenied(f"Missing required permissions: {', '.join(permissions)}")
